...
#                     
        ...
        for id_missing_record in ids_missing_records:
            ...
            if crm_time_start.date() < (datetime.now() - timedelta(days=1)).date():
            ...
            if files_transactions.get(str(transaction['id'])) == None and transaction['account']['title'] == 'Эквайринг Tinkoff' and item_ids.get(transaction['sold_item_id']) != None:
                ...
                if record_id == []:
                    try:
                        ...
                        do_log(f'TRANSACTION {client_id} {abonement_id} {amount} not in number_link!!!!') 
                    ...
                    do_log(f'TRANSACTION {client_id} {abonement_id} {amount} not record_id in transaction !!!!') 
        ...
        if counter == 240:
            ...
            do_log('DO_BACKUP: '+str(datetime.now()))
